...
android {
    ...
    signingConfigs {
        create("release") {
            keyAlias = extra["keyAlias"].toString()
            keyPassword = extra["keyPassword"].toString()
            storeFile = file(extra["storeFile"].toString())
            storePassword = extra["storePassword"].toString()
        }
    }
    buildTypes {
        release {
